...
struct FlowTable
{
	...
	spinlock_t tableLock;	//spinlock for this table
};
...
static void Init_Table(struct FlowTable* ft)
{
	...
	ft->size=0;
	//Initialize spinlock of the table
...
static unsigned int Insert_List(struct FlowList* fl, struct Flow* f)
{	
...
static unsigned int Insert_Table(struct FlowTable* ft,struct Flow* f)
{
		int result=0;
