...
Queue/MLFQ set queue_num_ $queue_num
Queue/MLFQ set thresh_ $DCTCP_K
Queue/MLFQ set dequeue_marking_ 1
...
for {set i 2} {$i < $node} {incr i} {
    ...
    $ftp($i) attach-agent $tcp($i)
    $ftp($i) set type_ FTP
    $ns at [expr 0.2+0.001*[expr rand()+1]*$i] "$ftp($i) send $data_size"
}
