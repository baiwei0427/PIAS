...
TcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer)
{
	...
	// Wei
	if (delay_bind(varName, localName, "lldct_", &lldct_ , tracer)) return TCL_OK;
...
void TcpAgent::opencwnd()
{
	...
	if (cwnd_ < ssthresh_) {
		...
		if(lldct_)
		{
			if(t_seqno_<200*1000) //If bytes sent < 200KB
			{
				lldct_w_c_=lldct_w_max_;
			}
			else if(t_seqno_>1000*1000) //If bytes sent > 1MB
			{
			...
			{
				//Reduce linearly
				lldct_w_c_=lldct_w_max_-(t_seqno_-200*1000)/800000*(lldct_w_max_-lldct_w_min_);
			}
		}
